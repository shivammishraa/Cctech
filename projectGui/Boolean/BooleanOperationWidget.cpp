...
bool compareFace2D(const std::vector<QVector2D>& a, const std::vector<QVector2D>& b, float tol = 0.0001f) {
    ...
    for (size_t i = 0; i < a.size(); ++i) {
        if ((a[i] - b[i]).length() > tol) return false;
    }
    return true;
}
...
void BooleanOperationWidget::performSubtraction() {
    ...
    result2D.clear();
    if (shapeA2D.empty() || shapeB2D.empty()) return;

    QPolygonF polyA, polyB;
    for (const auto& v : shapeA2D[0]) polyA << QPointF(v.x() + translationA2D.x(), v.y() + translationA2D.y());
    for (const auto& v : shapeB2D[0]) polyB << QPointF(v.x() + translationB2D.x(), v.y() + translationB2D.y());
    ...
    for (int i = 0; i < subPath.elementCount(); ) {
        ...
        while (i < subPath.elementCount()) {
            ...
            if (e2.isMoveTo()) break;
            ++i;
        }
        if (face.size() > 2) result2D.push_back(face);
    }
}
...
void BooleanOperationWidget::GLWidget::paintGL() {
    ...
    glLoadIdentity();
    if (!result2D.empty()) {
        for (const auto& face : result2D) renderShape2D(face, {0, 1, 0}, {0, 0}, 1.0f);
    } else {
        for (const auto& face : shapeA2D) renderShape2D(face, {1, 0, 0}, translationA2D, scaleA2D);
        for (const auto& face : shapeB2D) renderShape2D(face, {0, 0, 1}, translationB2D, scaleB2D);
    }
}
