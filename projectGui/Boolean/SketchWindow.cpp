    ...
    : QMainWindow(parent)
{
    ...
    connect(bezierBtn, &QPushButton::clicked, glWidget, &SketchGLWidget::toggleBezierMode);
...
void SketchWindow::handleIntersection() {
    if (glWidget->isBezierModeActive()) {
        glWidget->findBezierIntersections();
    } else {
        glWidget->performIntersection();
    }
}
